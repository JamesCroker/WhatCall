...
import { aliceScenarioDoc, aliceId, aliceScenarioPath } from "./testdata";

let testEnv: RulesTestEnvironment;

describe("Firestore Security Rules - Scenario", () => {
  beforeAll(async () => {
  ...
  it("should allow a user to read their own document", async () => {
    ...
    const docRef = doc(db, aliceScenarioPath);
  ...
  it("should allow a user to edit their own document", async () => {
    ...
    // 3. Test that the rule allows the read
    const docRef = doc(db, aliceScenarioPath);
    await assertSucceeds(setDoc(docRef, { ...aliceScenarioDoc, url: 'changed' }));
  });
  ...
  it("should deny a user from editing another user's document", async () => {
    ...
    const docRef = doc(db, aliceScenarioPath);
    await assertFails(setDoc(docRef, { ...aliceScenarioDoc, url: 'changed' }));
  });
  ...
  it("should deny a user from creating a document belonging to another user", async () => {
    ...
    const docRef = doc(db, aliceScenarioPath);
    await assertFails(setDoc(docRef, aliceScenarioDoc));
  });
